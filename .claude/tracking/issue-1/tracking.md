...
**Google sign-in (2026-09-14):** OAuth client (Web) created by the user in Google Cloud; redirect URI `https://dbedjxkhytvlvlkwdoma.supabase.co/auth/v1/callback`. The user saved the provider in the Supabase dashboard; verified through the Management API: enabled, client id matches, nonce check on, email not optional, signups still disabled, before-user-created hook still on. `/auth/v1/authorize?provider=google` → 302 to accounts.google.com with that client id, and Google accepts it (no `invalid_client` / `redirect_uri_mismatch`). Because signups are disabled, Google only signs in accounts that already exist (linked by verified email), so the six users must be created first. The end-to-end click-through (AC1.2) waits for the six Google addresses.
